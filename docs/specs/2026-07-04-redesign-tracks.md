...
**R2 결과 (2026-07-04, 오케스트레이터 검수 완료)**: 커밋 4c44656. 스프라이트 3장 구슬 교체(`atlas.png` 폐기), i18n 카피 개편(마법서→구슬, tome/spellbook/book→orb, 영어 관사·한글 조사 정합, 키명·플레이스홀더·data-testid·도감(CODEX) 유지 — composer 위임 후 검수), 테스트 갱신(`JourneyPanel`/`hints`/`BooksPanel` 카피 단언). 220 그린·tsc·build. 프리뷰: 탭 "구슬"·"첫 구슬 결속"·인벤 카피·슬롯 아이콘 구슬(`/assets/tomes` 16×16) 확인. Phaser 궤도 텍스처는 배포/캐시클리어 시 구슬(코드 무변경, 프리뷰는 브라우저 base 이미지 캐시로 옛것 잔상). `tome→orb` 코드 리네임은 보류(선택).

**통합 게이트 — 신선한 눈 실사용 (2026-07-04, 로컬 검증 완료)**: fresh 첫 화면 = 탭 1개(구슬)·빨간점 0·균열/랭킹 숨김(R1 언락 폭포)·구슬 카피(R2). 온보딩 = 구매→구슬 자동장착(holy L1·골드 27차감). 전직 세리머니(R5)·균열 전장 포탈(R4)·5탭 IA+랭킹 모달(R3)은 세이브 주입 실사용으로 검증됨. 밸런스 재증명 = R5 정직 지표(maxStageEver 57 / maxBookLevelEver 96) 4밴드 통과(첫벽 10m·첫환생 34m).
- **fe/04 auto-buy 확정: 폐기** (디렉터 결정). 기둥1 "머지가 왕"·R5 "자동클리어 금지"와 일관 — 자동화는 다음 머지의 재미를 깎는다. `useEngine` 잔존 상태·`04.json` 정리는 라이브 교체 시 일괄.
- **E2E 재작성·라이브 교체는 라이브(main) 트랙**: 원격 TestSprite·Vercel 배포는 이 브랜치 규칙상 미접근. main 교체 시점에 E2E 재작성(testid 변경: `tab-skills→tab-wizard`·`tab-quests→tab-journey`·`tab-ranks`제거→`ranks-btn`/`ranks-modal`, rift testid 유지, R2 copy) + 배포. **라이브 교체 판단 = 사용자 최종 승인.**

**남은 것**: 재설계 트랙 로컬 완주 완료(R1·R3·R4·R5코어+세리머니·R2 + 통합 게이트 로컬 검증). **라이브 교체(main 머지·Vercel 배포·E2E 재작성)는 사용자 승인 후 진행.**
